  ...
  return (
    <div className="space-y-5" role="region" aria-label="Widget palette">
      ...
      {Object.entries(groups).map(([groupName, types]) => (
        <div key={groupName}>
          <p className="text-[10px] font-medium uppercase tracking-wider text-[var(--text-muted)] mb-2 px-1">
            {groupName}
          </p>
            ...
            {types.map((type) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors duration-[var(--transition-fast)] hover:bg-[var(--bg-widget-hover)] cursor-grab active:cursor-grabbing"
                >
                  <span className="flex items-center justify-center w-8 h-8 rounded-md bg-[var(--bg-input)] shrink-0">
      ...
      <p className="text-[10px] text-[var(--text-muted)] px-1 pt-1">
